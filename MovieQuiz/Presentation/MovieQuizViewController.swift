...
final class MovieQuizViewController: UIViewController {
    ...
    @IBAction private func answerButtonPressed(_ sender: UIButton) {
        let userAnswer: Bool = sender.tag == 0 ? false : true
        let isCorrect: Bool = questions[questionNumber].correctAnswer == userAnswer
    ...
    private func showNextQuestionOrResults() {
        if questionNumber == questions.count - 1 {
          ...
          questionNumber += 1
          let nextQuestion: QuizStepViewModel = convert(model: questions[questionNumber])
    ...
    private func showQuizResult(quiz result: QuizResultsViewModel){
        ...
        let action = UIAlertAction(title: result.buttonText, style: .default) { _ in
            ...
            let firstQuestion = self.convert(model: self.questions[self.questionNumber])
